...
def test_runtime_evidence_bundle_writer_links_runtime_trace_and_contract_registry_reports(tmp_path) -> None:
    specification = {
    ...
    assert manifest_artifact.metadata["contract_registry_uri"].endswith("/contracts/runtime-contract-registry.json")
    ...
    assert [artifact.id for artifact in store.list()] == [
        "ARTIFACT-0001",
        "ARTIFACT-0002",
        "ARTIFACT-0003",
        "ARTIFACT-0004",
...
def test_runtime_evidence_bundle_writer_accepts_serialized_result_and_custom_manifest_path(tmp_path) -> None:
    ...
    assert manifest["artifacts"][2]["metadata"]["registry_version"] == "runtime-contracts/v1"
...
def test_runtime_evidence_manifest_verifier_reports_manifest_mismatches() -> None:
    verification = verify_runtime_evidence_manifest(
        {
            ...
            "artifacts": [
                ...
                {
                    "id": "ARTIFACT-0003",
                    "uri": "file:///contracts.json",
                    "metadata": {"artifact_role": "runtime_contract_registry", "registry_version": "wrong-version"},
                },
            ],
        }
    )
    ...
    assert verification.issues == [
        ...
        "Runtime evidence artifact 3 registry_version must match contract registry version.",
    ]
