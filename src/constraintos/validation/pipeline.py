...
class ValidationApprovalResult:
    """Combined validation and approval output for one artifact candidate."""
...
class ValidationApprovalPipeline:
    """Runs validation and then applies approval policy to the validation report."""

    def __init__(self, kernel: ValidationKernel | None = None, approval_engine: ApprovalEngine | None = None) -> None:
    ...
    def evaluate_render_specification(
        ...
        return ValidationApprovalResult(validation_report=validation_report, approval_decision=approval_decision)
